# Claude Code Skills — 소프트웨어 개발 자동화 플러그인 모음
...
```bash
npm install -g claude-code-skills
claude-code-skills
```
...
```
skills_vo/
├── install.sh                  ← 설치 스크립트 (진입점)
├── README.md                   ← 이 파일
├── github-kanban-skill/        ← 칸반 보드 스킬
├── write-prd-skill/            ← PRD 작성 스킬
├── write-techspec-skill/       ← TechSpec 작성 스킬
├── ci-cd-skill/                ← CI/CD 파이프라인 스킬
├── tddskill/                   ← TDD 워크플로우 스킬
└── github-flow-imp-skill/      ← GitHub 이슈 구현 스킬
```

> `write-prd/`, `write-techspec/` 폴더는 참고 자료 저장소이며 설치 대상에서 제외됩니다.
