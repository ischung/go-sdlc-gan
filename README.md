...
```bash
npx github:ischung/go-sdlc-gan                 # ~/.claude/ 에 설치
```
...
```bash
cd /your-project
npx github:ischung/go-sdlc-gan --project       # 현재 디렉토리의 .claude/ 에 설치
npx github:ischung/go-sdlc-gan --project /path # 지정 경로의 .claude/ 에 설치
```
...
```bash
npx github:ischung/go-sdlc-gan list              # 전역 설치 현황
npx github:ischung/go-sdlc-gan list --project    # 현재 디렉토리의 프로젝트 설치 현황
```

출력에는 **스킬 · 커맨드 · 서브에이전트** 세 섹션이 모두 표시되며, 각 항목이 `✔ / ✖`로 설치 여부를 보여줍니다. 서브에이전트 3종이 모두 `✔`가 아니면 `/implement`의 GAN 루프가 동작하지 않으니 `npx github:ischung/go-sdlc-gan` 재실행으로 보강하세요.
...
제거 후 Claude Code를 재시작하면 스킬이 비활성화됩니다. **`.claude/skills/`, `.claude/commands/`, `.claude/agents/`** 세 디렉토리의 관련 파일이 모두 제거되며, 프로젝트 스코프에서는 빈 디렉토리까지 자동 정리됩니다.
