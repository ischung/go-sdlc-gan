# go-sdlc-gan — 소프트웨어 개발 자동화 + GAN 품질 루프 Claude Code 스킬 패키지

소프트웨어 개발 전체 라이프사이클(기획 → 설계 → 이슈 관리 → 구현 → CI/CD)을 자동화하고, **로컬 구현 단계에 GAN(Generator-Evaluator) 품질 루프**를 적용한 **Claude Code 커스텀 스킬** 7종 + 3종 서브에이전트 패키지입니다.

> **go-sdlc-gan** = Go + Software Development Life Cycle + GAN quality loop

## 핵심 특징

- **AI 주도 SDLC 전 구간 자동화**: `/write-prd` → `/write-techspec` → `/generate-issues` → `/kanban-create` → `/implement` → `/ship` → `/cicd-pipeline` 한 흐름으로 연결.
- **배포 프로파일(L/W) 자동 전파**: PRD Phase 7에서 결정된 L(로컬 전용) / W(웹 배포) 선택이 TechSpec·이슈 발행·CI/CD 범위에 일관 적용.
- **GAN Generator-Evaluator 품질 루프** ✨ (신규): 로컬 구현 단계에서 **AC → TC 계약 잠금 → 구현 → 7항목 rubric 채점 → feedback 기반 재구현**을 최대 3회 반복. 판별자(Evaluator)는 **Opus 4.7**, 생성자(Generator)는 **Sonnet 4.6**로 모델을 분리하여 비용·품질 균형을 맞춤.
- **두 개의 독립 피드백 루프**: 로컬 GAN 루프(비즈니스 로직·사용자 가치) ↔ 원격 CI 루프(파이프라인·환경 호환성). 책임이 분리되어 실패 원인 추적이 쉬움.
- **교육적 투명성**: 모든 채점·판정 이력이 `docs/evaluations/issue-<N>/` 디렉토리에 문서로 축적 — 학생이 "왜 AI가 이 코드를 거부했는지"를 파일 diff로 추적 가능.
...
5. [GAN Generator-Evaluator 품질 루프](#5-gan-generator-evaluator-품질-루프-신규)
6. [스킬 비교](#6-스킬-비교)
...
**7개 스킬 + 12개 커맨드 + 3개 서브에이전트**가 설치됩니다. 설치 후 **Claude Code를 재시작**하세요.

> 서브에이전트(`sdlc-contracting`, `sdlc-code-generator`, `sdlc-code-evaluator`)는 `/implement`의 GAN 루프가 Task tool로 호출합니다. 자세한 내용은 [5. GAN 품질 루프](#5-gan-generator-evaluator-품질-루프-신규) 참고.

### 최신 버전 설치

```bash
# 전역 (모든 프로젝트에서 사용)
npx github:ischung/go-sdlc-gan

# 프로젝트 스코프 (해당 프로젝트에서만)
cd /your-project
npx github:ischung/go-sdlc-gan --project
npx github:ischung/go-sdlc-gan --project /path
```
...
npx github:ischung/go-sdlc-gan#v1.3.0                    # 전역
npx github:ischung/go-sdlc-gan#v1.3.0 --project          # 프로젝트 스코프

# npm 레지스트리 게시 후 사용 가능한 방식
npx go-sdlc-gan@1.0.0
npx go-sdlc-gan@1.0.0 --project
```

> 버전을 고정하면 팀원 전원이 동일한 스킬 버전을 사용하고, `list` 명령으로 설치 버전을 언제든 확인할 수 있습니다.

### 버전 비교

| 버전 | 주요 변경 | 비고 |
|------|-----------|------|
| **v1.3.0** | Task 툴 차단 해제 → GAN 에이전트 실제 동작 + Review 컬럼 생성 수정 + Done 직접 이동 | 현재 최신 |
| v1.3.0 | `/ship`·`/ship-all` GAN 루프 실제 실행 보장 — Task 호출 코드 직접 내장 | — |
| v1.1.0 | `/ship-all` GAN 루프 기본 활성화 + Review 컬럼 생성 버그 수정 | — |
| v1.0.1 | SKILL.md 단순화 — 중복 제거·과도한 설명 압축 (-244줄, 기능 동일) | — |
| v1.0.0 | GAN Generator-Evaluator 루프 + 서브에이전트 3종 도입 | — |
...
└── agents/                 ← GAN 루프용 서브에이전트 (신규)
    ├── sdlc-contracting.md      (model: sonnet)  AC → TC 번역 · sprint-contract.md 생성
    ├── sdlc-code-generator.md   (model: sonnet)  feedback 반영 구현/재구현
    └── sdlc-code-evaluator.md   (model: opus)    7항목 rubric 채점 · qa-report-iter-K.md 생성
```
...
```bash
npx github:ischung/go-sdlc-gan list              # 전역 설치 현황 + 버전
npx github:ischung/go-sdlc-gan list --project    # 프로젝트 스코프 설치 현황 + 버전
```

출력 예시:

```
  ✔  설치된 버전: v1.3.0  (설치일: 2026-04-19)
  ...
  ✔  스킬: github-flow-impl
       ✔  /implement
       ✔  /impl
  ...
  에이전트 (GAN Generator-Evaluator):
  ✔  sdlc-contracting
  ✔  sdlc-code-generator
  ✔  sdlc-code-evaluator
```

설치된 버전(`v1.3.0`)과 현재 패키지 버전이 다르면 **업데이트 경고**가 표시됩니다. 서브에이전트 3종이 모두 `✔`가 아니면 GAN 루프가 동작하지 않으니 재설치하세요.
...
**역할**: 칸반 보드 이슈를 선택하여 GitHub Flow 방식으로 구현하고 PR을 생성합니다. CI/CD 모니터링 없이 **PR 생성에서 종료**합니다. **GAN Generator-Evaluator 품질 루프가 내장**되어 있어, `/implement` 한 번 호출이 "계약 잠금 → 구현 → 채점 → feedback 재구현"을 자동으로 수행합니다.
...
| 플래그 | 동작 | 용도 |
|--------|------|------|
| `--no-eval` | Step 4.5/5.5/5.6 건너뜀 — 기존 단순 경로 | GAN 비활성, 레거시 호환 |
| `--eval-threshold <N>` | rubric 평균 임계값 조정 (기본 **8.0**) | 교육용 완화, 컨테스트 강화 |
| `--eval-max-iter <N>` | Generator↔Evaluator 최대 반복 (기본 **3**) | 더 많은 재시도 허용 |
| `--eval-economy` | Evaluator도 Sonnet — 비용 1/3 | 실습 수업용 |
| `--eval-strict` | Generator도 Opus — 최고 품질 | 졸업작품·컨테스트 |

**실행 흐름** (GAN 루프가 포함된 전체)
...
Step 4    코드 구현 (Generator, iter = 1)  ← sdlc-code-generator 호출
  └─ Step 4-0  E2E 이슈 감지 시 앱 분석 → playwright 테스트 자동 생성 (↓ 상세)
  └─ Step 4-1  일반 구현 (AC 또는 이슈 설명 기준)
Step 4.5  Contracting (AC → TC 잠금)  ← sdlc-contracting 호출
          → docs/evaluations/issue-<N>/sprint-contract.md 생성 + 사용자 1회 승인
Step 5    로컬 테스트 (build + test, 결과 보존)
Step 5.5  Evaluator (QA 채점)  ← sdlc-code-evaluator 호출
          → docs/evaluations/issue-<N>/qa-report-iter-<K>.md 생성
Step 5.6  분기
  ├─ PASS            → Step 6 (PR 생성 + qa-report 궤적 첨부)
  ├─ FAIL (iter < 3) → Step 4 재진입 (feedback 주입, iter+1)
  └─ FAIL (iter == 3 or plateau) → 사용자 회부
Step 6    PR 생성 + 칸반 이동: In Progress → Review
Step 7    완료 보고 ("PR 머지 시 Done 자동 이동", GAN 궤적 포함)
```

> GAN 루프 상세(rubric · 파일 구조 · 모델 할당 · 교육 가치)는 [5. GAN 품질 루프](#5-gan-generator-evaluator-품질-루프-신규) 참고.
...
> **GAN 루프와의 관계**: `/ship`은 `github-flow-impl`의 **Step 4.5/5.5/5.6(로컬 GAN 루프)**을 **그대로 위임**한 뒤, PR 오픈 후 **원격 CI 루프(Step 7)**에 진입합니다. 두 루프는 **독립된 카운터**를 가지므로 각자 최대 3회 재시도합니다 — 같은 "3회"가 중복 소모되지 않습니다.
>
> | 루프 | 책임 | 재시도 |
> |---|---|---|
> | 로컬 GAN 루프 (Step 4.5/5.5/5.6) | 비즈니스 로직 · 사용자 가치 · rubric 채점 | `IMPL_ITER_MAX=3` |
> | 원격 CI 루프 (Step 7) | 파이프라인 구조 · 빌드 환경 호환성 · 의존성 | `RETRY_COUNT=3` |
...
Step 5    로컬 테스트 (build + test 결과 보존)            │ github-flow-impl에
Step 5.5  Evaluator (QA 채점, qa-report-iter-K.md)       │ 그대로 위임
Step 5.6  분기: PASS → Step 6 / FAIL → Step 4 재진입      ┘ (로컬 GAN 루프)
Step 6    PR 생성 (※ Review 칸반 이동 없음 — 자동 머지까지 진행)
Step 7    CI/CD 모니터링 루프  ← 원격 CI 루프
  └─ CI 없으면 → Step 8로 스킵
  └─ CI 있으면 → 30초 간격 폴링
       ├─ 통과 → Step 8
       └─ 실패 → 피드백 루프 (최대 3회, 로컬 GAN 루프와 독립)
...
| 브랜치 생성 | ✅ | ✅ |
| E2E 테스트 자동 생성 | ✅ | ✅ |
| **GAN 로컬 루프 (Contracting + Evaluator)** | ✅ | ✅ (위임) |
| **Generator ↔ Evaluator 재시도** | ✅ (최대 3회) | ✅ (최대 3회) |
| 로컬 테스트 | ✅ | ✅ |
| PR 생성 | ✅ | ✅ |
| 칸반 Review 이동 | ✅ | ❌ (자동 머지까지 가므로) |
| CI/CD 모니터링 | ❌ | ✅ |
| 실패 로그 자동 분석 | ❌ | ✅ |
| 원격 CI 자동 수정 재시도 | ❌ | ✅ (최대 3회, GAN 루프와 독립) |
| 자동 squash 머지 | ❌ | ✅ |
...
## 5. GAN Generator-Evaluator 품질 루프 (신규)

### 5.1 왜 도입했나

`/implement`의 기존 품질 게이트는 **로컬 `npm test` 1회 실행**이 전부였습니다. 이는 다음을 놓칩니다:

- **비즈니스 로직 버그** — 테스트에 명시되지 않은 AC 구현 누락
- **사용자 가치 부재** — 코드는 컴파일되지만 이슈가 해결하려던 사용자 행동이 여전히 불가능
- **Scope Creep** — 이슈 범위 밖 파일을 불필요하게 수정
- **Silent regression** — 기존 테스트를 깨는 변경

이 모든 문제가 원격 CI에 올라가서야 드러나면 수정 비용이 급증합니다. **GAN(Generative Adversarial Network) 스타일의 Generator-Evaluator 루프**는 로컬 단계에서 이를 먼저 거릅니다.

### 5.2 루프 개요

```mermaid
flowchart TB
    A[이슈 #N] --> B["sdlc-contracting (sonnet)<br/>AC → TC 번역"]
    B --> C["sprint-contract.md 잠금<br/>(사용자 1회 승인)"]
    C --> D["sdlc-code-generator (sonnet)<br/>iter 1: 최초 구현"]
    D --> E["npm run build + npm test<br/>로그 보존"]
    E --> F["sdlc-code-evaluator (opus 4.7)<br/>7항목 rubric 채점"]
    F --> G["qa-report-iter-K.md"]
    G --> H{"PASS?<br/>평균 ≥ 8.0 AND<br/>각 항목 ≥ 6.0"}
    H -->|PASS| I[PR 생성<br/>궤적 첨부]
    H -->|FAIL, K < 3| J["feedback 추출<br/>iter K+1로 재진입"]
    J --> D
    H -->|FAIL, K == 3<br/>or plateau| K[사용자 회부]
```
...
| 에이전트 | 모델 | 역할 | 입력 → 출력 |
|---------|------|------|-------------|
| **sdlc-contracting** | `sonnet` (Sonnet 4.6) | "엔지니어링 계약 협상가" — 이슈 AC를 테스트 가능한 명제(TC)로 번역 | 이슈 본문 + AC → `sprint-contract.md` (TC 5~10개, Non-goals, DoD) |
| **sdlc-code-generator** | `sonnet` (Sonnet 4.6) | "실행력 있는 시니어 개발자" — 계약 충족 코드 생성 또는 feedback 반영 재구현 | sprint-contract.md (+ 이전 qa-report) → 소스 파일 수정 + build/test 실행 |
| **sdlc-code-evaluator** | `opus` (Opus 4.7) | "까다로운 QA" — 실제 명령 출력을 인용하며 rubric 채점 | sprint-contract.md + git diff + 로그 → `qa-report-iter-K.md` (점수 + feedback) |

> **모델 분리 근거**: GAN 이론의 핵심은 **Discriminator가 Generator보다 최소 같거나 강해야** 학습이 수렴한다는 것입니다. 판별자의 오판(false positive·false negative)은 루프 전체를 망치므로 Evaluator에 Opus 4.7을 배정했습니다. Generator는 "판단보다 실행"이 중심이므로 Sonnet 4.6으로 비용·속도를 확보합니다.

### 5.4 평가 기준 (7항목 rubric)

| # | 항목 | 설명 | 배점 근거 |
|---|------|------|-----------|
| 1 | **TC Pass Rate** | sprint-contract TC 통과율 | `floor(pass/total × 10)` |
| 2 | **Build & Test Health** | `npm run build`, `npm test` 종료 코드 + 경고 | exit 0 + warnings 0 → 10 |
| 3 | **AC Coverage** | AC의 핵심 키워드가 변경 파일에서 실제로 다뤄졌는가 | 키워드-파일 매핑 증거 |
| 4 | **Code Hygiene** | 기존 컨벤션(네이밍·디렉토리·import) 준수, 린트 위반 수 | `10 - min(warnings, 5)` |
| 5 | **Scope Discipline** | Non-goals 침범 여부, diff 범위가 이슈와 일치하는가 | 침범 파일 수로 감점 |
| 6 | **Regression Safety** | 기존 테스트 파손 수, 대량 삭제 감지 | 파손 1건 × -3 |
| 7 | **User Value Trace** | 이슈가 해결하려던 사용자 행동이 실제로 가능해졌는가 (E2E 1개 상상 실행) | 실행 가능성 판단 |

**PASS 조건**: 평균 ≥ **8.0** AND 모든 항목 ≥ **6.0**.
**모든 점수는 실제 명령 출력 또는 diff 인용을 근거로 필요** — 주관적 형용사("깔끔하다", "좋아 보인다") 금지.

### 5.5 파일 기반 상태 (중단·재개 가능)

이슈별로 다음 디렉토리가 쌓입니다:

```
docs/evaluations/issue-42/
├── sprint-contract.md           ← Contracting 단계에서 잠금 (이후 불변)
├── qa-report-iter-1.md          ← 1차 채점 (FAIL + feedback)
├── qa-report-iter-2.md          ← 2차 채점 (FAIL + feedback)
└── qa-report-iter-3.md          ← 3차 채점 (PASS 또는 plateau)
```

- Claude의 컨텍스트 창에 의존하지 않으므로 **세션을 새로 시작해도 루프를 이어갈 수 있습니다**.
- PR 본문에 qa-report 점수 **궤적**(예: `5.6 → 7.2 → 8.1`)이 자동 첨부됩니다.

### 5.6 루프 제어 상수

| 상수 | 기본값 | 플래그 | 의미 |
|------|--------|--------|------|
| `IMPL_ITER_MAX` | `3` | `--eval-max-iter` | Generator ↔ Evaluator 최대 반복 횟수 |
| `EVAL_THRESHOLD` | `8.0` | `--eval-threshold` | rubric 평균 임계값 |
| `EVAL_ITEM_MIN` | `6.0` | (고정) | 각 항목의 하한 |
| `PLATEAU_EPS` | `0.3` | (고정) | iter간 평균 상승폭 — 미만이면 정체로 간주, 조기 중단 |

### 5.7 비용 감각 (이슈 1건당 추정)

| 구성 | Generator | Evaluator | 대략 비용 | 용도 |
|------|:---------:|:---------:|:--------:|------|
| 기본 | Sonnet | Opus | **~$1.9** | 수업 실습 표준 |
| `--eval-economy` | Sonnet | Sonnet | ~$1.2 | 비용 최소화, 저학년 실습 |
| `--eval-strict` | Opus | Opus | ~$6.0 | 졸업작품·컨테스트 |

> iteration 수 × 모델별 토큰가를 조합한 개략치입니다. 실제 비용은 이슈 크기와 수렴 속도에 따라 변동합니다.

### 5.8 교육적 가치

GAN 루프는 소프트웨어 공학의 핵심 교훈 **"생성과 평가의 분리"**를 파일 이력으로 드러냅니다.

- 학생이 `docs/evaluations/issue-N/` 디렉토리 diff를 추적하면 **"AI가 왜 자기 코드를 거부했는지"**를 문서로 볼 수 있습니다.
- AC가 어떻게 TC로 번역되는지(`sprint-contract.md`), 같은 코드가 rubric 축마다 어떻게 다른 점수를 받는지, feedback 기반 재구현이 수렴/정체하는 양상을 **숫자로 실감**합니다.
- 수업 활동 예: 학생에게 `qa-report-iter-1.md`를 읽히고 "평가자가 지적한 3가지를 1회 구현으로 고칠 수 있을지 가설을 세우고, iter-2에서 확인"하도록 과제화.

### 5.9 `--no-eval` (escape hatch)

GAN 루프를 완전히 건너뛰려면:

```bash
/implement #42 --no-eval
```

Step 4.5/5.5/5.6이 제거되고, 기존 `/implement`의 **단순 경로**(로컬 테스트 1회 재시도 후 PR)로 실행됩니다. 서브에이전트가 설치되지 않은 환경의 긴급 탈출구로 사용하세요.
...
```mermaid
flowchart TD
    A[이슈 선정] --> B[코드 구현<br/>iter = 1]
    B --> C{E2E 이슈?}
    C -->|YES| D[앱 분석<br/>→ playwright 테스트 자동 생성]
    D --> E[Contracting<br/>AC → TC 잠금]
    C -->|NO| E
    E --> F[로컬 테스트]
    F --> V[Evaluator<br/>7항목 rubric 채점]
    V --> W{PASS?}
    W -->|FAIL, iter<3| B
    W -->|FAIL, iter=3| X[사용자 회부]
    W -->|PASS| PR[PR 생성]

    PR --> G{스킬 종류}
    G -->|implement| H[칸반 → Review<br/>사람이 머지]
    G -->|ship| I[CI/CD 폴링]
    I --> J{CI 결과}
    J -->|통과| K[자동 squash 머지<br/>칸반 → Done]
    J -->|실패| L[로그 분석<br/>자동 수정<br/>최대 3회<br/>GAN 루프와 독립]
    L --> I
```
...
7단계: /ship-all           → [L0]→[L1]→[L2]→[L3]→기능 이슈 순서로 전체 자동화
                              (각 이슈는 내부적으로 GAN 루프로 품질 검증)
       /ship               → 단일 이슈 CI/CD 통과 + 자동 머지
                              (로컬 GAN 루프 + 원격 CI 루프, 독립 재시도)
       /implement          → 단일 이슈 PR 생성까지 (GAN 루프 + 사람이 리뷰 후 머지)
       /tdd                → 각 기능을 TDD로 검증
```

> `ship-all` · `/ship` · `/implement` 모두 **로컬 GAN 품질 루프**를 거친 뒤에만 PR을 오픈합니다. 루프 결과(`qa-report-iter-K.md` 궤적)가 PR 본문에 자동 첨부됩니다. 자세한 내용은 [5. GAN 품질 루프](#5-gan-generator-evaluator-품질-루프-신규) 참고.
...
# 특정 버전 제거
npx github:ischung/go-sdlc-gan#v1.3.0 uninstall

# 프로젝트 스코프 제거 (현재 디렉토리)
npx github:ischung/go-sdlc-gan uninstall --project

# 특정 버전 + 프로젝트 스코프 제거
npx github:ischung/go-sdlc-gan#v1.3.0 uninstall --project

# 지정 경로 제거
npx github:ischung/go-sdlc-gan uninstall --project /path
```

제거 후 Claude Code를 재시작하면 스킬이 비활성화됩니다. **`.claude/skills/`, `.claude/commands/`, `.claude/agents/`** 세 디렉토리의 관련 파일과 버전 메타 파일(`go-sdlc-gan.json`)이 모두 제거되며, 프로젝트 스코프에서는 빈 디렉토리까지 자동 정리됩니다.

> `docs/evaluations/` 디렉토리는 **제거되지 않습니다** — 이전 이슈의 sprint-contract 및 qa-report 이력은 보존됩니다.
...
| **github-flow-impl** | `/implement` `/impl` | 이슈 구현 → PR 생성 (**GAN 루프 + E2E 이슈 자동 생성** 내장) |
| **auto-ship** | `/ship` `/ship-all` | 구현 + **로컬 GAN 루프** + 원격 CI 피드백 루프 + 자동 squash 머지 end-to-end 자동화 |

## 포함된 서브에이전트 (3종, 신규)

| 에이전트 | 모델 | 호출 위치 | 역할 |
|---------|:----:|----------|------|
| **sdlc-contracting** | `sonnet` | `/implement` Step 4.5 | AC → TC(Testable Criterion) 번역 후 `sprint-contract.md` 잠금 |
| **sdlc-code-generator** | `sonnet` | `/implement` Step 4 / 재진입 시 | 최초 구현 또는 이전 qa-report feedback 반영 재구현 |
| **sdlc-code-evaluator** | `opus` | `/implement` Step 5.5 | 7항목 rubric 채점 + `qa-report-iter-K.md` 생성 (Discriminator 역할) |

> 세 에이전트는 **파일 기반 상태 공유**(`docs/evaluations/issue-<N>/`)로 통신하므로 세션이 끊겨도 중단점부터 재개할 수 있습니다. 자세한 루프 동작은 [5. GAN 품질 루프](#5-gan-generator-evaluator-품질-루프-신규) 참고.
